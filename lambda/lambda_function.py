...
class CallPhoneIntentHandler(AbstractRequestHandler):
    
    def can_handle(self, handler_input):
        return is_intent_name("CallPhoneIntent")(handler_input)
    
    def handle(self, handler_input):
        language_prompts = handler_input.attributes_manager.request_attributes["_"]
        
        speech_output = random.choice(language_prompts['CALL_PHONE'])
        reprompt = random.choice(language_prompts['CALL_PHONE_REPROMPT'])
...
class ChooseContactIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        try:
            ...
            for record in phonebook:
                if record['contact_name'] == recipient_name:
                    call = client.calls.create(
                        twiml = '<Response><Say>This is an automated call from Alexa to find your phone.</Say></Response>',
                        to = '+1' + record['phone_number'],
                        from_ = callers_phone_number
                    )
                    speech_output = random.choice(language_prompts['RINGING_PHONE'])
                    reprompt = random.choice(language_prompts['RINGING_PHONE_REPROMPT'])
                    break
                else:
            ...
            speech_output = random.choice(language_prompts['COULDNT_RING_PHONE'])
            reprompt = random.choice(language_prompts['COULDNT_RING_PHONE_REPROMPT'])
...
sb.add_request_handler(CallPhoneIntentHandler())
